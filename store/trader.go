...
type Trader struct {
	...
	// 跟单配置
	CopyProviderType   string  `json:"copy_provider_type"`
	CopyProviderParams string  `json:"copy_provider_params"` // JSON string
	CopyRatio          float64 `json:"copy_ratio"`
	CopyMinNotional    float64 `json:"copy_min_notional"`
	CopyMaxNotional    float64 `json:"copy_max_notional"`
	CopyFollowOpen     bool    `json:"copy_follow_open"`
	CopyFollowAdd      bool    `json:"copy_follow_add"`
	CopyFollowReduce   bool    `json:"copy_follow_reduce"`
	CopyFollowClose    bool    `json:"copy_follow_close"`
	CopyLeverageSync   bool    `json:"copy_leverage_sync"`
	CopyMarginSync     bool    `json:"copy_margin_sync"`
	CopyPriceFallback  bool    `json:"copy_price_fallback"`
...
func (s *TraderStore) initTables() error {
	_, err := s.db.Exec(`
		CREATE TABLE IF NOT EXISTS traders (
			...
			copy_provider_type TEXT DEFAULT '',
			copy_provider_params TEXT DEFAULT '',
			copy_ratio REAL DEFAULT 0,
			copy_min_notional REAL DEFAULT 0,
			copy_max_notional REAL DEFAULT 0,
			copy_follow_open BOOLEAN DEFAULT 0,
			copy_follow_add BOOLEAN DEFAULT 0,
			copy_follow_reduce BOOLEAN DEFAULT 0,
			copy_follow_close BOOLEAN DEFAULT 0,
			copy_leverage_sync BOOLEAN DEFAULT 0,
			copy_margin_sync BOOLEAN DEFAULT 0,
			copy_price_fallback BOOLEAN DEFAULT 0,
	...
	alterQueries := []string{
		...
		`ALTER TABLE traders ADD COLUMN copy_provider_type TEXT DEFAULT ''`,
		`ALTER TABLE traders ADD COLUMN copy_provider_params TEXT DEFAULT ''`,
		`ALTER TABLE traders ADD COLUMN copy_ratio REAL DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_min_notional REAL DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_max_notional REAL DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_follow_open BOOLEAN DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_follow_add BOOLEAN DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_follow_reduce BOOLEAN DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_follow_close BOOLEAN DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_leverage_sync BOOLEAN DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_margin_sync BOOLEAN DEFAULT 0`,
		`ALTER TABLE traders ADD COLUMN copy_price_fallback BOOLEAN DEFAULT 0`,
	}
...
func (s *TraderStore) Create(trader *Trader) error {
	...
	`, trader.ID, trader.UserID, trader.Name, trader.AIModelID, trader.ExchangeID, trader.StrategyID,
		...
		trader.CopyProviderType, trader.CopyProviderParams, trader.CopyRatio, trader.CopyMinNotional, trader.CopyMaxNotional,
		trader.CopyFollowOpen, trader.CopyFollowAdd, trader.CopyFollowReduce, trader.CopyFollowClose,
		trader.CopyLeverageSync, trader.CopyMarginSync, trader.CopyPriceFallback)
	return err
}
...
func (s *TraderStore) List(userID string) ([]*Trader, error) {
	rows, err := s.db.Query(`
		SELECT id, user_id, name, ai_model_id, exchange_id, COALESCE(strategy_id, ''),
		       ...
		       COALESCE(copy_provider_type, ''), COALESCE(copy_provider_params, ''), COALESCE(copy_ratio, 0),
		       COALESCE(copy_min_notional, 0), COALESCE(copy_max_notional, 0),
		       COALESCE(copy_follow_open, 0), COALESCE(copy_follow_add, 0),
		       COALESCE(copy_follow_reduce, 0), COALESCE(copy_follow_close, 0),
		       COALESCE(copy_leverage_sync, 0), COALESCE(copy_margin_sync, 0), COALESCE(copy_price_fallback, 0),
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&t.CopyProviderType, &t.CopyProviderParams, &t.CopyRatio, &t.CopyMinNotional, &t.CopyMaxNotional,
			&t.CopyFollowOpen, &t.CopyFollowAdd, &t.CopyFollowReduce, &t.CopyFollowClose,
			&t.CopyLeverageSync, &t.CopyMarginSync, &t.CopyPriceFallback,
			&createdAt, &updatedAt,
		)
		if err != nil {
			return nil, err
...
func (s *TraderStore) Update(trader *Trader) error {
	_, err := s.db.Exec(`
		UPDATE traders SET
			...
			copy_provider_type = ?, copy_provider_params = ?, copy_ratio = ?,
			copy_min_notional = ?, copy_max_notional = ?, copy_follow_open = ?,
			copy_follow_add = ?, copy_follow_reduce = ?, copy_follow_close = ?,
			copy_leverage_sync = ?, copy_margin_sync = ?, copy_price_fallback = ?,
			updated_at = CURRENT_TIMESTAMP
		WHERE id = ? AND user_id = ?
	`, trader.Name, trader.AIModelID, trader.ExchangeID, trader.StrategyID,
		trader.ScanIntervalMinutes, trader.IsCrossMargin,
		trader.CopyProviderType, trader.CopyProviderParams, trader.CopyRatio,
		trader.CopyMinNotional, trader.CopyMaxNotional, trader.CopyFollowOpen,
		trader.CopyFollowAdd, trader.CopyFollowReduce, trader.CopyFollowClose,
		trader.CopyLeverageSync, trader.CopyMarginSync, trader.CopyPriceFallback,
		trader.ID, trader.UserID)
	return err
}

// UpdateCopyParams 更新跟单参数（游标）
func (s *TraderStore) UpdateCopyParams(userID, id string, params string) error {
	_, err := s.db.Exec(`
		UPDATE traders SET copy_provider_params = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ? AND user_id = ?
	`, params, id, userID)
	return err
}
