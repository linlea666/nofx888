...
import type { AIModel, Exchange, CreateTraderRequest, Strategy, CopyTradingConfig, TraderConfigData } from '../types'
...
export function TraderConfigModal({
  ...
  const [formData, setFormData] = useState<FormState>({
    ...
    exchange_id: traderData?.exchange_id || availableExchanges[0]?.id || '',
    ...
    copy_config: traderData?.copy_config || {
      provider_type: '',
      provider_params: {},
      copy_ratio: 100,
      min_notional: 0,
        ...
        >
          ...
          <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-[#EAECEF] mb-4 flex items-center gap-2">
              <span className="text-[#F0B90B]">0</span> 定比跟单配置
            </h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={
                    (copyCfg?.provider_params as any)?.uniqueName ||
                    (copyCfg?.provider_params as any)?.address ||
                    ''
                  }
                  onChange={(e) =>
                    updateCopyConfig({
