...
type CopyConfig struct {
	...
	StaleEventWindowSec   int64             `json:"stale_event_window_sec"`            // 事件最大允许延迟秒数，超时则丢弃（0 表示不校验，默认 600）
...
func (c *CopyConfig) EnsureDefaults() {
	...
	if c.StaleEventWindowSec == 0 {
		c.StaleEventWindowSec = 600 // 默认 10 分钟内的事件有效
	}
