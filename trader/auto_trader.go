...
type AutoTraderConfig struct {
	...
	// Copy trading 配置（可选，存在则启用领航员比例跟单）
	CopyConfig *copysync.CopyConfig
}
...
// SetCopyConfig 设置 CopySync 配置（必须在 Run 前调用）。
func (at *AutoTrader) SetCopyConfig(cfg *copysync.CopyConfig) {
	at.copyConfig = cfg
	at.config.CopyConfig = cfg
}
...
func (at *AutoTrader) Run() error {
	...
	if at.config.CopyConfig != nil {
		...
		if err != nil {
			...
			if err := at.copyService.Start(); err != nil {
				logger.Infof("❌ [%s] 跟单服务启动失败: %v", at.name, err)
			} else {
				logger.Infof("🔗 [%s] 跟单服务已启动 (provider=%s)", at.name, at.config.CopyConfig.ProviderType)
			}
		}
	}

	// 如果是跟单模式，则不再执行 AI 决策循环，仅保持服务存活
	if at.copyService != nil {
		for at.isRunning {
			select {
			case <-at.stopMonitorCh:
				logger.Infof("[%s] ⏹ 跟单模式停止", at.name)
				return nil
			}
		}
	} else {
...
func (at *AutoTrader) Stop() {
	...
	at.isRunning = false
	// 持久化 provider 游标
	if at.copyService != nil && at.store != nil && at.config.CopyConfig != nil && at.config.CopyConfig.ProviderParams != nil {
		cursor := at.copyService.ProviderCursor()
		if cursor > 0 {
			params := at.config.CopyConfig.ProviderParams
			params["last_seq"] = fmt.Sprintf("%d", cursor)
			paramsJSON, _ := json.Marshal(params)
			// 更新 DB copy_provider_params
			_ = at.store.Trader().UpdateCopyParams(at.userID, at.id, string(paramsJSON))
		}
		at.copyService.Stop()
		at.copyService = nil
...
func (at *AutoTrader) GetPositions() ([]map[string]interface{}, error) {
	...
	for _, pos := range positions {
		...
		marginUsed := (quantity * markPrice) / float64(leverage)
		if m := toFloat(pos["margin"]); m > 0 {
			marginUsed = m
		} else if m := toFloat(pos["marginUsed"]); m > 0 {
			marginUsed = m
		}
