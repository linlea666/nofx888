...
func (s *DecisionStore) initTables() error {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS decision_records (
			...
			follower_equity REAL DEFAULT 0,
			follower_notional REAL DEFAULT 0,
			follower_qty REAL DEFAULT 0,
...
func (s *DecisionStore) scanDecisionRecord(rows *sql.Rows) (*DecisionRecord, error) {
	...
	record.PriceSource = priceSource.String
	record.FollowerEquity = followerEquity.Float64
	record.FollowerNotional = followerNotional.Float64
	record.FollowerQty = followerQty.Float64
	record.CopyRatio = copyRatio.Float64
	record.Formula = formula.String
